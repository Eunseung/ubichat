# ubichat — IA·플로우·정책 개선안

> **원본 복사 기준:** [`_archive/03_기존_루트_기획자료_20260727/02_상담시스템_IA_플로우.md`](./_archive/03_기존_루트_기획자료_20260727/02_상담시스템_IA_플로우.md) (유학생 상담 플랫폼 — IA·플로우 v3)
> **개정 목적:** 원본은 보존하고, 2026-07-21 기준의 ubichat 공개 대학 탐색 서비스와 역할별 진입 정책을 반영한다.
> **목업 적용 위치:** 프로젝트 루트

## 1. 서비스 정의와 원칙

ubichat의 공개 랜딩은 내부 보고용 홈이 아니라, 학생 관점에서 전문대학을 탐색하는 **공개 서비스 홈**이다. 대학과 기업 광고 구좌는 이 공개 탐색 경험 안에 배치한다.
...
|---|---|---|---|---|
| 비로그인 | ubichat 엠블럼 + `ubichat` | 대학 탐색 · 상담하기 | 학교 서비스 · 학생 로그인 | 문서 보관함 · 프로필 메뉴 |
| 로그인 학생 | ubichat 엠블럼 + `ubichat` | 대학 탐색 · 내 상담 · 콘텐츠 | 학생 프로필 메뉴 | 학교 서비스 · 학생 로그인 |

- `대학 탐색`은 공개 탐색 서비스의 목록 화면으로 연결한다.
- 비로그인 사용자가 `상담하기`를 선택하면 학생 로그인으로 이동하고, 성공 후 상담 화면으로 복귀한다.
- 로그인 학생의 프로필 메뉴는 `내 상담`, `공지사항`, `문서 보관함`, 기존 학생 화면의 `마이페이지`, `로그아웃`을 제공한다.
- 축소 화면은 주 GNB, 학생 이름·이메일, 개인 메뉴, 다국어 선택을 햄버거 하나에 통합하고 별도 프로필·다국어 버튼은 숨긴다.
...
| ubichat 부가 서비스 | TOPIK 등 자사 서비스 | 기업 제휴 배너 이후 서비스 안내 | 공개 랜딩 |
...
|---|---|---|
| 공개 랜딩 | ubichat 브랜드, 공개 탐색 중심 GNB, 학생/대학 진입 분리, 활성 광고 구좌 캐러셀 반영 | 배너 소재 교체는 AM-5 운영 업무 |
| 학생 | GNB를 공개 탐색 정책과 연결, 프로필 메뉴에 마이페이지·로그아웃 배치 | 상담·문서·마이페이지 본문 IA는 유지 |
| 대학 | ubichat 엠블럼·`대학 서비스` 컨텍스트로 로그인/운영 상단 정렬, 학생 로그인과 분리 | 대학 콘솔의 본문 업무 IA는 유지 |
| 관리자 | ubichat 엠블럼·`플랫폼 관리자` 컨텍스트로 로그인/운영 상단 정렬, 단일 대학 데이터 및 공개/미노출 상태 설명, AM-5 광고 구좌 관리 | 운영 콘솔의 본문 업무 IA는 유지 |
...
- [ ] 학생 로그인 후 `대학 서비스`와 `로그인`은 보이지 않고, 프로필 메뉴에 `공지사항`, `문서 보관함`, `마이페이지`, `로그아웃`이 보인다.
- [ ] 축소 화면의 햄버거에서 페이지 메뉴, 학생 프로필·개인 메뉴, 다국어 선택을 모두 사용할 수 있고 밖에 중복 버튼이 없다.

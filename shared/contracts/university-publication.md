...
## 구미대학교 저장 스냅샷 호환 — D-027

- 기존 `unichat.mock.gumi-publication.v1` 값은 삭제·초기화하지 않는다.
- 새 저장값은 `schemaVersion`을 포함한다. 구 schema는 읽을 때만 호환 변환하며, 공개 소비 화면은 변환 결과를 저장소에 다시 쓰지 않는다.
- 탭 호환: 기존 `dormitory`는 본문·순서를 유지한 `dormitory-intro`/`기숙사 소개`로 읽는다. 누락된 `dormitory-photos`는 사진 배열 `[]`, 기본 `enabled: false`로 보완한다.
- 보존: 기존 탭의 제목·본문·순서·활성 상태와 `입학 안내 자료·입학 일정` 같은 추가 탭은 유지한다.
- 새 schema 저장: 학교의 `직접 게시`만 수행한다. 소비자는 구·신 schema를 모두 읽고 렌더링만 한다.

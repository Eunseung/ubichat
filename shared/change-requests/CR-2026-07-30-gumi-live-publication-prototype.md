...
## 기숙사 기본 더미·정적 사진 — 2026-07-30

- 적용: 구미대학교 `undergraduate`의 `기숙사` 공개 탭 기본값.
- 본문: 생활관 안내, 정원·선발·문의, 2026학년도 1학기 관리비 표, 식사·부대시설.
- 정적 사진 참조: `squads/university/prototype-assets/gumi/gumi-dormitory-01.png`부터 `04.png`까지 4개.
- 메인 소비자는 `profile.dormitoryPhotos`의 기존 정적 참조만 캐러셀로 렌더링한다. 직접 게시 시 새 로컬 업로드·객체 URL은 D-026 범위 밖이므로 저장하지 않는다.

## 지원 과정 기본 더미 — 2026-07-30

- 적용: 구미대학교 `undergraduate`의 `admission-process` 공개 탭 기본값.
- 2027학년도 수시 1·2차, 정시·자율모집의 원서접수·면접·합격자 발표 표와 외국인(유학생) 전형 지원 자격을 포함한다.
- 기존 `publicDetailTabs` 본문 필드만 사용한다. 소비자 데이터 모델·이벤트는 변경하지 않는다.
